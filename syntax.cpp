...
string SyntaxAnalyzer::_CONSTDECLARATION(SyntaxTreeNode* TreeNode)
{
    ...
    _stable.addEntry(_lexicalOutput[_currentWordIndex - 1]._value,"CONST");
...
string SyntaxAnalyzer::_UNSIGNEDINTEGER(SyntaxTreeNode* TreeNode)
{
    ...
    TreeNode->addChild(childNode);
    string temp = _lexicalOutput[_currentWordIndex]._value;
...
string SyntaxAnalyzer::_VARDECLARATION(SyntaxTreeNode* TreeNode)
{
    ...
    _stable.addEntry(_lexicalOutput[_currentWordIndex-1]._value,"VAR");
    while (_lexicalOutput[_currentWordIndex]._value == ",") {
        ++_currentWordIndex;
        SyntaxTreeNode* childNode2 = new SyntaxTreeNode(COMMA, ","); 
        TreeNode->addChild(childNode2);
...
string SyntaxAnalyzer::_ASSIGNMENTSTATEMENT(SyntaxTreeNode* TreeNode)
{
    ...
    if (_lexicalOutput[_currentWordIndex]._type == "IDENTIFIER") {
        ...
        // 获取标识符的值
        string identifierValue = _lexicalOutput[_currentWordIndex]._value;
        ...
        childNode->setValue(_lexicalOutput[_currentWordIndex]._value);
        TreeNode->addChild(childNode);
        ++_currentWordIndex;
    }
    else {
...
string SyntaxAnalyzer::_EXPRESSION(SyntaxTreeNode* TreeNode)
{
    ...
    if (_lexicalOutput[_currentWordIndex]._value == "+" ||
        ...
        op1 = _lexicalOutput[_currentWordIndex]._value;
        isaddorsub = true;
...
string SyntaxAnalyzer::_EXPRESSION2(SyntaxTreeNode* TreeNode) {
    ...
    string temp = _lexicalOutput[_currentWordIndex-1]._value;
    if (_lexicalOutput[_currentWordIndex]._value == "+" ||
        _lexicalOutput[_currentWordIndex]._value == "-") {
...
string SyntaxAnalyzer::_TERM2(SyntaxTreeNode* TreeNode)
{
    if (_lexicalOutput[_currentWordIndex]._value == "*" ||
        ...
        _stable.addEntry(lastTemporary(), "MID");
        string currenttemp = lastTemporary();
        emit(temp1 = currenttemp + multiplicativeoperator + factor);
...
string SyntaxAnalyzer::_LOOPSTATEMENT(SyntaxTreeNode* TreeNode)
{
    ...
    labelValues[labelBegin] = instuctioncount - 1;
    ...
    labelValues[labelEnd] = instuctioncount - 1;
    return "";
}
